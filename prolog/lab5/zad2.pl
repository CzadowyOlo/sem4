...
%setting every two field as white color
get_tile_color(RowNumber, ColNumber, Color) :-
    Rest is mod((RowNumber + ColNumber), 2),
    (Rest = 1 -> Color = white; Color = black),!.

%drawing content of fields black/white x empty/full-----------------
draw_tile_with_queen(white) :- write('| ### ').

draw_tile_with_queen(black) :- write('|:###:').

draw_tile_without_queen(white) :- write('|     ').

draw_tile_without_queen(black) :- write('|:::::').
%and borders of chessplate:
